...
```
run_tier(tier):
  SET LOCAL lock_timeout = profile.lock_timeout            -- e.g. 100ms
  SET LOCAL statement_timeout = profile.job_timeout(tier)  -- INVARIANT: strictly < tier interval
  ...
  FOR target IN capture_plan(tier) ORDER BY manifest order:
    BEGIN                                                    -- per-target EXCEPTION block (subtransaction)
      SET LOCAL statement_timeout = target.section_timeout   -- e.g. 250ms -- set explicitly EVERY iteration: SET LOCAL
                                                               -- only reverts automatically if this block raises; on the
                                                               -- happy path it persists forward into the next target
...
- **Overrun protection** is the invariant `job_timeout(tier) < tier_interval(tier)` — enforce it with a CHECK/assertion when profiles are applied. This replaces the circuit breaker (§8).
- `VERIFY-DURING-IMPLEMENTATION №1`: confirm pg_cron's actual behavior when an invocation overruns its schedule slot (skip vs. concurrent launch) on the supported pg_cron versions, and note it in docs. Do not assume; test it. The timeout invariant should make the question moot, but the semantics must be stated from evidence.
...
- **Profiles** — survive, reduced to *cadence + bounds*: `default` (fast 1m / medium 5m / slow 15m / on_change 5m; lock_timeout 100ms; section 250ms) and `troubleshooting` (fast 15–30s; medium 1m; looser section timeouts). `apply_profile(name)` remaps tiers and bounds; never touches the manifest.
