...
| **Gauge** | Instantaneous level read at snapshot time | Exact at the sample instant, undefined between samples | `datfrozenxid_age`, `datminmxid_age`, `db_size_bytes`, `connections_active`, xmin-horizon ages on the `snapshots` compat view |
...
The per-column classification is not maintained in this document. It lives in the schema itself, with each extension annotating its own objects: every user-facing view column carries a `COMMENT ON COLUMN` with a machine-parseable `[class] [units]` prefix (in both schemas, though most are on pgfr_record's views), and pgfr_analyze's set-returning functions carry an `Output columns:` block in their function comment. The parsed registry, `pgfr_analyze.column_semantics()` (one row per column: relation, column name, semantic class, units, interval basis, notes), ships with the optional pgfr_analyze extension and reads the annotations from both schemas. The comments are the source of truth for what any given column means; `\d+` on a view shows them inline, so a core-only install (pgfr_record without pgfr_analyze) still has every annotation, just not the registry function that parses them.
...
4. **Circuit-breaker and cron gaps (missingness that correlates with load).** `pgfr_record._check_circuit_breaker()` skips a collection pass when the recent average runtime of that collector exceeds a threshold (by default, the last 3 successful runs averaged over a 15-minute window against `circuit_breaker_threshold_ms` = 1000). Load shedding does the same for the ring sampler when the active-backend fraction is high (the snapshot job has no load-shedding check and relies on the circuit breaker alone), and pg_cron itself can miss ticks on an overloaded system. The consequence: samples go missing *because* the system was under stress. Gaps are informative missingness (MNAR, missing not at random), correlated with exactly the states you are trying to observe. Gaps are first-class data. They must be shown, attributed, and never silently interpolated over.
...
`pgfr_analyze.coverage(start, end)` reports expected versus observed ticks per collector at the fixed one-minute cadence, and `pgfr_analyze.coverage_gaps(start, end)` lists each gap with an attributed reason: `retention_horizon` (the tick predates the oldest evidence still retained), `circuit_breaker` and `load_shedding` (skip evidence recorded at collection time), `restart` (the unobserved run leading into the postmaster start time), `cron_inactive`, or `unknown`. `pgfr_analyze.report()` qualifies its conclusions when any collector's retention-adjusted coverage falls below **90%** for the window (retention truncation is a resolution limit, not missed collection, so it does not count against confidence). Gaps attributed to the circuit breaker or load shedding always trigger the missing-not-at-random caveat, in `report()` and the incident-analysis reports alike, regardless of the overall ratio: collection was skipped because the system was under stress, so absence of samples is not absence of activity.
