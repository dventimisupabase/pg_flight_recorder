...
- **No adaptive safety mechanisms.** No circuit breaker, no load shedding, no mode switching. Just static bounds (`lock_timeout` per target, `job_timeout` per tier run) and a capture ledger that records every miss with its reason. Bounded, recorded degradation beats an instrument that goes dark exactly when an incident makes it most valuable, and `pg_cron`'s own overrun behavior (it serializes rather than piling up concurrent runs of an overrunning job) has been confirmed against a live instance to make those static bounds sufficient on their own.
...
- [pgfr_analyze](../pgfr_analyze/README.md): the optional reporting/analysis extension, not yet rebuilt for v2.
