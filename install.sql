...
BEGIN
    ...
    -- Vacuum control: Safety mode (critical)
    FOR v_row IN
        SELECT vcr.schemaname, vcr.relname, vcr.mode_reason
        FROM flight_recorder.vacuum_control_report(p_start_time, p_end_time) vcr
        WHERE vcr.operating_mode = 'safety'
        LIMIT 5
    LOOP
        anomaly_type := 'VACUUM_CONTROL_SAFETY_MODE';
        severity := 'critical';
        description := format('Table %s.%s in safety mode: %s',
               v_row.schemaname, v_row.relname, v_row.mode_reason);
        metric_value := 'Safety mode active';
        threshold := 'XID age >50% freeze_max_age or blocking transactions';
        recommendation := 'Urgent: address vacuum blocking issues to prevent XID wraparound.';
        RETURN NEXT;
    END LOOP;

    -- Vacuum control: Catch-up mode (high)
    FOR v_row IN
        SELECT vcr.schemaname, vcr.relname, vcr.mode_reason
        FROM flight_recorder.vacuum_control_report(p_start_time, p_end_time) vcr
        WHERE vcr.operating_mode = 'catch_up'
        LIMIT 5
    LOOP
        anomaly_type := 'VACUUM_CONTROL_CATCHUP_MODE';
        severity := 'high';
        description := format('Table %s.%s in catch-up mode: %s',
               v_row.schemaname, v_row.relname, v_row.mode_reason);
        metric_value := 'Catch-up mode active';
        threshold := 'Dead tuples growing, budget exhaustion imminent';
        recommendation := 'Review vacuum tuning recommendations from vacuum_control_report().';
        RETURN NEXT;
    END LOOP;
...
BEGIN
    ...
    -- ==========================================================================
    -- Vacuum Control Recommendations Section (v2.8)
    -- ==========================================================================
    v_result := v_result || '## Vacuum Control Recommendations' || E'\n\n';

    SELECT count(*) INTO v_count
    FROM flight_recorder.vacuum_control_report(p_start_time, p_end_time)
    WHERE should_recommend = true OR operating_mode IN ('safety', 'catch_up');

    IF v_count = 0 THEN
        v_result := v_result || '(no recommendations - vacuum keeping up with workload)' || E'\n\n';
    ELSE
        v_result := v_result || '| Table | Mode | Diagnostic | Current SF | Recommended SF | Change | Action |' || E'\n';
        v_result := v_result || '|-------|------|------------|------------|----------------|--------|--------|' || E'\n';
        FOR v_row IN
            SELECT vcr.schemaname, vcr.relname, vcr.operating_mode,
                   vcr.diagnostic_classification, vcr.current_scale_factor,
                   vcr.recommended_scale_factor, vcr.change_pct, vcr.alter_table_sql
            FROM flight_recorder.vacuum_control_report(p_start_time, p_end_time) vcr
            WHERE vcr.should_recommend = true OR vcr.operating_mode IN ('safety', 'catch_up')
            ORDER BY
                CASE vcr.operating_mode WHEN 'safety' THEN 1 WHEN 'catch_up' THEN 2 ELSE 3 END
            LIMIT 10
        LOOP
            v_result := v_result || '| ' ||
                COALESCE(v_row.schemaname || '.' || v_row.relname, '-') || ' | ' ||
                COALESCE(v_row.operating_mode, '-') || ' | ' ||
                COALESCE(v_row.diagnostic_classification, '-') || ' | ' ||
                COALESCE(v_row.current_scale_factor::TEXT, '-') || ' | ' ||
                COALESCE(v_row.recommended_scale_factor::TEXT, '-') || ' | ' ||
                COALESCE(v_row.change_pct::TEXT || '%', '-') || ' | ' ||
                CASE WHEN v_row.alter_table_sql IS NOT NULL THEN 'Tune SF' ELSE '-' END || ' |' || E'\n';
        END LOOP;
        v_result := v_result || E'\n';

        -- Add ALTER TABLE statements for copy-paste
        v_result := v_result || '**Recommended SQL:**' || E'\n\n';
        v_result := v_result || '```sql' || E'\n';
        FOR v_row IN
            SELECT vcr.alter_table_sql
            FROM flight_recorder.vacuum_control_report(p_start_time, p_end_time) vcr
            WHERE vcr.alter_table_sql IS NOT NULL
              AND (vcr.should_recommend = true OR vcr.operating_mode IN ('safety', 'catch_up'))
            LIMIT 10
        LOOP
            v_result := v_result || v_row.alter_table_sql || E'\n';
        END LOOP;
        v_result := v_result || '```' || E'\n\n';
    END IF;
