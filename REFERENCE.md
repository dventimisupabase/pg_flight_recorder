...
| `kind` | `text` | `'capture'` (an archive's full captured column set) or `'rollup'` (an endpoint-shape rollup's narrower counter/odometer-only column set, see [Rollups](#rollups)); one `source_view` can have a dictionary row of each kind at once, never the same row |
...
- **Endpoint** (Group B, counters/odometers): one row per `(bucket, key)`, storing the first and last observed value of every counter/odometer column in that bucket, the two points a reset-aware delta needs. `first_values`/`last_values` are dictionary-encoded jsonb arrays, not name-keyed objects: `schema_id` points at a `'rollup'` kind `payload_schemas` row (a second, narrower dictionary alongside the archive's own `'capture'` kind row for the same `source_view`, holding only the counter/odometer column subset) minted alongside `rollup_close_sql` by `generate_capture_plan()`, the same mint-together invariant the capture schema already keeps. `first_reset_values`/`last_reset_values` stay name-keyed objects (a different, usually much narrower column list). `schema_id` is nullable: a row written before this dictionary existed has `schema_id NULL` and keeps its original object-encoded shape forever, never rewritten -- `rollup_deltas()` decodes either shape depending on whether a given row has one, the same tolerance `generate_presentation_views()` already has for an archive with more than one schema generation. Column order (for the array-encoded shape) follows `payload_schemas`' own order. No primary key, matching the archive tables' own convention: uniqueness is the collector's bucket-close discipline (below), not a database constraint.
- **Stat** (Group C, gauges): one row per `(bucket, stat_name)`, aggregated across every key in the bucket, since Group C's value is "did this happen in this bucket", not a per-backend/per-slot history. Which statistic to compute is a judgment call, seeded in `pgfr_record.rollup_specs(source_view, stat_name, agg, value_expr, predicate_sql)` for fourteen of Group C's fifteen targets, the same maintenance posture as `column_classes`' own override list. The exception, `pg_stat_wal_receiver`, needs no entry: its own LSN odometer columns make it mechanically eligible for the endpoint shape instead, the same rule Group B uses. `generate_rollups()` checks `rollup_specs` before `column_classes`, so a target's explicit hand-picked stat always wins over an incidental odometer column (e.g. `pg_stat_replication` and `pg_replication_slots` also carry LSN odometers, but keep the stat shape their own `rollup_specs` rows specify). Its `value` column is `double precision`, not `numeric`: every stat here is already an error-bounded Mode A estimate (see `STATISTICS.md`), not an exact figure, so arbitrary precision buys nothing and a fixed-width float is cheaper across a rollup table's row count. `generate_rollups()` migrates an existing table's `value` column to `double precision` in place if it finds the original `numeric`, so this applies on re-run, not just a fresh install.
...
`pg_stat_activity` additionally gets one generated `rollup_specs` row per `(wait_event_type, wait_event)` pair the running PostgreSQL version's `pg_wait_events` catalog knows about (PG17+), rather than a hand-picked subset: every wait event this PostgreSQL version defines gets its own `sample_count` stat, the same Mode A time-in-state estimate as the `pg_stat_progress_*` targets' `active_sample_count` (see `STATISTICS.md`). `stat_name` is `wait_<type>_<name>`, lowercased, with `type` included because PostgreSQL itself reuses one name (`SyncRep`) across two types (`IPC` and `LWLock`). On PostgreSQL 15 and 16, which predate `pg_wait_events`, `pg_stat_activity`'s rollup carries only the two hand-seeded duration stats above.

`pg_wait_events` has no counterpart for the other enumerable text columns on Group C targets, so their per-value breakdowns are hand-seeded instead of generated: `pg_stat_activity.state` (six values, `state_<value>`), `pg_locks.locktype` (twelve values including `applytransaction`, PG16+ only but harmless as a never-matching predicate on earlier versions, `locktype_<value>`), and both `pg_stat_replication.state` (five values, `state_<value>`) and `sync_state` (four values, `sync_state_<value>`). Each is a `sample_count` stat, same Mode A reasoning as the wait-event rows above. These sit alongside each target's original hand-picked stat (`blocked_sample_count`, `non_streaming_sample_count`, etc.), which stays for continuity rather than being replaced.
...
None of these functions read `pgfr_record`'s rollup tables or call `rollup_deltas()` (see [Rollups](#rollups)): every function below is bounded by a target's own raw `retention`, not its longer `rollup_retention`. A window reaching further back than raw retention returns whatever the raw archive still has, which for a fully-aged-out Group B or C target is nothing, not an automatic fallback to the rollup. Query `pgfr_record.rollup_deltas()` directly (Group B targets only; it raises on a stat-shaped Group C target, which has no delta to compute) for a longer, coarser look than any function here currently provides.
