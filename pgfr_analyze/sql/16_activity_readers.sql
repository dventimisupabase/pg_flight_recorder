...
-- current-state reads via state_as_of() (no capture is ever "the" vacuum
-- or transaction; it's whatever is in flight as of p_t); wal_archiver_status()
-- is a deltas() window like every other Group A rate/count check.
...
BEGIN
    ...
    v_sql := format(
        ...
        FROM pgfr_record.state_as_of(%2$L, %1$L::timestamptz) AS d(%3$s)
...
COMMENT ON FUNCTION pgfr_analyze.long_running_transactions(timestamptz, interval) IS
    'Backends whose transaction has been open longer than p_threshold (default 5 minutes) as of p_t (default now), from pg_stat_activity via state_as_of(), any state -- broader than anomaly_report()''s IDLE_IN_TRANSACTION check, which only flags backends idle inside their transaction.';
