...
**90-day/weekly-aggregated window (Issue #92, in progress)**: a second window for real seasonality handling -- "business workloads breathe on a 7-day cycle" -- aggregating by rolling 7-day bucket rather than by day. `_refresh_consumption_trends_weekly()` is a deliberate sibling to `_refresh_consumption_trends()`, not a unified rewrite (see that function's own file header for why), sharing the same `consumption_trends` table (`window_days = 84` distinguishes these rows -- 12 complete weeks, not literally 90 days, since a partial final week would reintroduce the distortion aggregating exists to avoid) and the same `consumption_trend_min_r2` / `consumption_trend_step_r2_margin` thresholds, gated by its own `consumption_trend_min_weeks` (default 8). No composition-drift guard at this grain yet, and `consumption_trend_report()` doesn't show it yet either -- both are still-open phases of #92.
